...
- When rendering `Image` from `@yext/visual-editor`, you likely want to pass `className="h-full"` so the image fills its wrapper.
...
return <Image image={resolvedLogoImage} className="h-full" />;
```

## defaultProps Rules

- `defaultProps.logoImage.constantValue` must be the plain the plain image object shape (`{ url, width, height }`), which is valid because `TranslatableAssetImage` is a union
