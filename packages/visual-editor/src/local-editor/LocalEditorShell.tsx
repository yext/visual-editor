...
export const LocalEditorShell = ({
  ...
  const activeLayoutOptions = supportedLayoutIds;
  const hasAnyEntities = React.useMemo(() => {
    return Object.values(manifest?.entitiesByLayout ?? {}).some((entities) => {
      return entities.length > 0;
    });
  }, [manifest?.entitiesByLayout]);
  const controlsDisabled = isManifestLoading || !activeLayoutOptions.length;
  const editorKey = `${selectedLayoutId}:${selectedLocale}:${selectedMode}`;
  const shouldRenderEditorFrame =
    isDocumentLoading || (!!documentResponse?.document && !!selectedLayoutId);
  ...
  const openPreview = React.useCallback(() => {
    ...
    setPreviewContext({
      config: activeConfig,
      defaultLayoutData: selectedLayoutDefaults?.defaultLayoutData,
      streamDocument,
    ...
    documentResponse,
    editorLocalDevOptions,
    selectedLayoutDefaults?.defaultLayoutData,
    streamDocument,
  ]);
  ...
  return (
    ...
    >
      ...
      >
        <div>
          ...
          <p style={{ margin: "8px 0 0", color: "#555" }}>
            Switch layouts, entities, and locales against local snapshot data.
          </p>
        </div>
        <div
          style={{
      ...
      <LocalEditorControls
        ...
        activeLayoutOptions={activeLayoutOptions}
        ...
        onLayoutChange={(layoutId) => {
          updateSearchParam("layoutId", layoutId);
        }}
      />
      ...
      {!isManifestLoading &&
        ...
        activeLayoutOptions.length === 0 && (
          <LocalEditorNotice
            title="No Supported Layouts"
            body="The local editor could not find any generated Section Library layouts that match the available component registry."
            tone="warning"
          />
        )}
