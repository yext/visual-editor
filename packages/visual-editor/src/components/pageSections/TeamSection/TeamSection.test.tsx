...
describe("TeamSection", async () => {
  ...
  it("resolves a linked team source before any card mappings are selected", async () => {
    const data = {
      root: {
        props: {
          version: migrationRegistry.length,
      ...
      content: [
        {
          ...
          props: {
            ...
            slots: {
              ...TeamSection.defaultProps!.slots,
              CardsWrapperSlot: [
                {
                  type: "TeamCardsWrapper",
                  props: {
                    ...((TeamSection.defaultProps!.slots as any)
                      .CardsWrapperSlot[0].props as any),
                    data: {
  ...
  it("resolves linked team title mappings without hanging", async () => {
    const data = {
      root: {
        props: {
          version: migrationRegistry.length,
    ...
    const resolved = await resolveAllData(data, puckConfig, {
      streamDocument: {
        c_teamSection: {
          people: [
            { name: "Mr Tomato", title: "Fruit" },
            { name: "Mr Potato", title: "Vegetable" },
          ],
        },
      },
    });
