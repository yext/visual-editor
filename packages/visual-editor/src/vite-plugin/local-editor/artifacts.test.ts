...
describe("createLocalEditorArtifactsManager", () => {
  it("threads the repo migration registry through the local editor", () => {
  ...
  it("uses an empty migration registry when the repo file is absent", () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "local-editor-"));
    rootDirs.push(rootDir);
    const artifacts = createLocalEditorArtifactsManager({
      localEditorTemplateSource:
  ...
  it("warns when layout IDs generate the same data template name", () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "local-editor-"));
    rootDirs.push(rootDir);
    ...
    expect(warn).toHaveBeenCalledWith(
      expect.stringContaining(
        'Local Editor layouts "foo_bar" and "foo-bar" generate the same data template'
      )
    );
  });
