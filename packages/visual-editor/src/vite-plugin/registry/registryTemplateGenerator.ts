 ...
 *    - Write that JSON string into the matching template's
 *      `defaultLayoutData`.
...
const buildConfigSource = (
  ...
  const configPropsName = `Generated${configExportName}Props`;
  const componentProps = items.map((item) => {
    return `  ${JSON.stringify(item.componentName)}: any;`;
  });
  ...
  return [
    ...
    "",
    `type ${configPropsName} = Pick<MainConfigProps, "MainContent"> & {`,
    ...componentProps,
    "};",
    "",
    `export const ${configExportName}: Config<${configPropsName}> = {`,
...
const updateTemplateManifest = (
  ...
  for (const templateName of templateNames) {
    ...
    if (!existingEntry) {
      manifest.templates.push({
        ...
        exampleSiteUrl: "",
        layoutRequired: true,
        defaultLayoutData: JSON.stringify(defaultLayoutData),
      });
      updated = true;
      continue;
    }

    if (existingEntry.defaultLayoutData !== JSON.stringify(defaultLayoutData)) {
      existingEntry.defaultLayoutData = JSON.stringify(defaultLayoutData);
      updated = true;
    }
  }
  ...
  if (effectiveTemplateNames.usesFallbackMain && !fallbackMainEntry) {
    manifest.templates.push({
      ...
      exampleSiteUrl: "",
      layoutRequired: true,
      defaultLayoutData: JSON.stringify(DEFAULT_LAYOUT),
    });
    updated = true;
  } else if (
    effectiveTemplateNames.usesFallbackMain &&
    fallbackMainEntry &&
    fallbackMainEntry.defaultLayoutData !== JSON.stringify(DEFAULT_LAYOUT)
  ) {
    fallbackMainEntry.defaultLayoutData = JSON.stringify(DEFAULT_LAYOUT);
    updated = true;
  }
