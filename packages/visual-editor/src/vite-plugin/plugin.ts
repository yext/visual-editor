...
import editCompatTemplate from "./templates/editCompat.tsx?raw";
...
export const yextVisualEditorPlugin = (): Plugin => {
  ...
  const syncDevEditEntryTemplate = (
    rootDir: string,
    compatTemplateInfo: ReturnType<
      typeof getEditorCompatTemplateInfoFromTemplateNames
    >,
    options?: {
      preserveExistingEditTemplate?: boolean;
    }
  ) => {
    const compatTemplatePath = path.join(
      rootDir,
      "src",
      "templates",
      "edit.tsx"
    ...
    if (!compatTemplateInfo) {
      if (options?.preserveExistingEditTemplate) {
        return;
      }

      if (isAutogeneratedTemplateFile(compatTemplatePath)) {
        fs.removeSync(compatTemplatePath);
      }
      return;
    }

    fs.mkdirSync(path.dirname(compatTemplatePath), { recursive: true });

    if (
      fs.existsSync(compatTemplatePath) &&
      !isAutogeneratedTemplateFile(compatTemplatePath)
    ) {
      throw new Error(
        `Refusing to overwrite hand-authored compatibility template at ${compatTemplatePath}`
      );
    }

    writeFileIfChanged(
      compatTemplatePath,
      injectEditorCompatTemplateInfo(editCompatTemplate, compatTemplateInfo)
    );
    trackFileForCleanup(compatTemplatePath);
  };
   ...
   * 3. Sync the generated editor templates and dev entry template with the
  ...
  const generateFiles = () => {
    ...
    const compatTemplateInfo = isBuildMode
      ? null
      : getEditorCompatTemplateInfoFromTemplateNames(availableTemplateNames);
    const editorTemplateInfos = syncEditorInstanceTemplates(
      rootDir,
      availableTemplateNames,
      compatTemplateInfo ? ["src/templates/edit.tsx"] : []
    );
    syncDevEditEntryTemplate(
      rootDir,
      ...
      isBuildMode
        ...
        : Array.from(
            new Set([
              ...
              ...(compatTemplateInfo ? [compatTemplateInfo.configName] : []),
            ])
          )
    );
