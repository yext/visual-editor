...
import {
  readSharedSectionRegistry,
  type SharedSection,
} from "./sharedSectionRegistry.ts";
...
const readSectionLibrary = (rootDir: string): SectionLibrary | undefined => {
  ...
  const sharedRegistry = readSharedSectionRegistry(
    path.join(libraryDirectory, "shared", "sectionRegistry.ts")
  );
  const sharedSections = sharedRegistry?.sections ?? [];
  ...
  if (
    ...
    throw new Error(
      `Missing shared section registry at ${path.join(libraryDirectory, "shared", "sectionRegistry.ts")}`
    );
  }
  validateComponentIds(sections, sharedSections);
  for (const layout of layouts) {
    validateLayoutReferences(layout, sections, sharedSections);
  }
  return {
    metadata,
    sections,
...
const validateComponentIds = (
  ...
  for (const section of [...sections, ...sharedSections]) {
    if (!isSafeId(section.id)) {
      throw new Error(`Section ID is not valid: ${section.id}`);
    }
    if (componentIds.has(section.id)) {
      throw new Error(`Section ID is not unique: ${section.id}`);
    }
    componentIds.add(section.id);
  }
};
...
const validateLayoutReferences = (
  ...
  const componentIds = new Set(
    [...sections, ...sharedSections]
      .filter((section) =>
        section.pageSetTypes.includes(layout.metadata.pageSetType)
      )
...
const buildConfigSource = (
  ...
  const hasSharedRegistry =
    sharedSections.some((section) => {
      return section.pageSetTypes.includes(layout.metadata.pageSetType);
  ...
  return sectionLibraryConfigTemplate
    ...
    .replace(
      ...
      hasSharedRegistry
        ? 'import { sharedComponents, sharedRootAllowedComponentIds, sharedRootConfigs, sharedSections } from "../shared/sectionRegistry";'
        : 'const sharedSections: { id: string; pageSetTypes: string[] }[] = [];\nconst sharedComponents: Config["components"] = {};\nconst sharedRootAllowedComponentIds: Partial<Record<string, string[]>> = {};\nconst sharedRootConfigs: Partial<Record<string, NonNullable<Config["root"]>>> = {};'
    )
    .replace(
    ...
    .replace(
      "__SECTION_LIBRARY_HAS_SHARED_REGISTRY__",
      hasSharedRegistry ? "true" : "false"
    );
};
