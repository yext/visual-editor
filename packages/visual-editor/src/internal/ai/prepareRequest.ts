...
const prepareField = (field: Record<string, any>): Record<string, any> => {
  ...
  if (field.type in yextAiFieldTypes) {
    const aiFieldType =
      yextAiFieldTypes[field.type as keyof typeof yextAiFieldTypes];
    return mergeAiConfig(annotatedField, {
      schema: aiFieldType.schema,
      instructions: aiFieldType.description,
      ...(field.type === "testImage" ? { stream: false } : {}),
    });
  }
