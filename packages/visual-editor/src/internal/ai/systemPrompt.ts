...
The component registration must contain label, html, styles, fields, and defaultProps. Every data-puck-field-* HTML annotation must have a quoted JSON value, for example data-puck-field-title='{ "type": "testEntityField" }'. Never emit a bare attribute such as data-puck-field-title. Each annotation contains only one supported field type and has matching fields and defaultProps entries with the same name. Move authored text into transform-backed defaultProps and leave annotated HTML targets empty. Every testImage field definition must include ai: { stream: false } so Puck never applies a partial image value. Follow each supplied field type's schema and description. Use one balanced root element.
...
Create the requested self-contained component. Repeated static content is allowed when every editable value has a unique test* field annotation with a quoted JSON type value. Leave annotated HTML targets empty. Do not use slots or child components.
`
  .replaceAll(/\s+/g, " ")
  .trim();
