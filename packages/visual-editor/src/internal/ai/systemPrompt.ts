...
The component registration must contain label, html, styles, fields, and defaultProps. Every data-puck-field-* HTML annotation contains only one supported field type and has a matching fields entry and defaultProps entry with the same name. Follow each supplied field type's schema and description for those entries. Use one balanced root element and do not include streaming.
...
Create the requested self-contained component. Repeated static content is allowed when every editable value has a unique test* field annotation and matching fields/defaultProps entries. Do not use slots, child components, nested CTA/image bindings, or streaming registrations.
`
  .replaceAll(/\s+/g, " ")
  .trim();
